...
const EmployeeView = () => {
  ...
  const { data: employeesList, error, isLoading } = useGetEmployeesByCurrentItemQuery(currentItem)
  const totalOfIsActive = employeesList?.filter(employee => employee.isActive).length ?? 0
  const totalOfEmployees = employeesList?.length ?? 0

  return (
    <>
      <div className="flex flex-col gap-5">
        <Button variant={"filter"} className="w-full py-8" onClick={() => dispatch(setToggleComponent("form"))}>
          + Adicionar
        </Button>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <span className="text-muted-foreground text-sm">Ativos {totalOfIsActive} / {totalOfEmployees}</span>
        </div>
      </div>
